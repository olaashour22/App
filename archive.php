...
//echo $current_cats->name; 
$subcategories 		= get_categories(array( 'parent' => $current_cats->term_id ) );
$subcat_noparent	= get_categories(array( 'parent' => '' )); 
if ( $subcategories === FALSE or $subcategories == '' ) $subcategories = $subcat_noparent;
?>
    ...
    <div class="page-head-cat">
        <div class="wrp-min">
            <div class="head-cat-title">
                ...
                <?php if(!empty($subcategories)){ ?><?php } ?>
				<div class="head-cat-tools">
                    <button class="cat-menu-btn collapsed" type="button" title="<?php _e('Select a category', THEMES_NAMES); ?>" data-toggle="collapse" data-target="#collapse-menu"><svg width="24" height="24"><use xlink:href="#i__settings"></use></svg><span class="sr-only"><?php _e('Select a category', THEMES_NAMES); ?></span></button>
                </div>
            ...
            <div class="cat-menu-collapse collapse" id="collapse-menu">
                <div class="cat-menu-list">
                    <ul> 					
					<?php 
					//echo do_shortcode( '[list_subcats]' );  
					if($subcategories){				 
						foreach($subcategories as $cat){
						$catLink 		= get_term_link($cat->term_id); 
						$classactive 	= ( is_category( $cat->name ) ) ? ' active' : '';
						echo '<li class="catalog-genres-item'.$classactive.'"><a href="'. $catLink.'">'.$cat->name.'</a></li>'; 
						}  
					} else {
						foreach($subcat_noparent as $cat){
						$catLink 		= get_term_link($cat->term_id); 
						$classactive 	= ( is_category( $cat->name ) ) ? ' active' : '';
						echo '<li class="catalog-genres-item'.$classactive.'"><a href="'. $catLink.'">'.$cat->name.'</a></li>'; 
						}  
