...
if ( post_password_required() ) { ?>
    <p class="nocomments"><?php _e('This post is password protected. Enter the password to view comments.', THEMES_NAMES); ?></p>
    <?php
    return;
}
...
<span><svg width="24" height="24"><use xlink:href="#i__arrowright"></use></svg><span class="sr-only"><?php _e('back', THEMES_NAMES); ?></span></span>
</span>
<?php }}
...
<span class="page_prev" title="back">
<span><svg width="24" height="24"><use xlink:href="#i__arrowleft"></use></svg><span class="sr-only"><?php _e('back', THEMES_NAMES); ?></span></span>
</span>
<?php }}
...
function get_next_comments_links( $label = '', $max_page = 0 ) {
	...
	if ( empty( $label ) ) {
		$label = __( '<svg width="24" height="24"><use xlink:href="#i__arrowright"></use></svg><span class="sr-only">next</span>', THEMES_NAMES );
	} 
...
function get_previous_comments_links( $label = '' ) {
	...
	if ( empty( $label ) ) {
		$label = __( '<svg width="24" height="24"><use xlink:href="#i__arrowleft"></use></svg><span class="sr-only">back</span>', THEMES_NAMES );
	} 
...
<span class="group-label <?php if ($user_id) { $user_info = get_userdata($user_id ); ?>g-adm<?php } else { ?>g-guest<?php } ?>"><?php if ($user_id) { $user_info = get_userdata($user_id ); echo implode(' ', $user_info->roles); } else { ?><?php _e('Guests', THEMES_NAMES); ?><?php  } ?></span>
<time class="date c-muted" datetime="<?php printf(  __( '%1$sT%2$s', THEMES_NAMES ), get_comment_date(), get_comment_time() ); ?>" style="color: #8a949d !important;"><?php printf(  __( '%1$s', THEMES_NAMES ), get_comment_date(), get_comment_time() ); ?> <?php edit_comment_link(__('(Edit)'),'  ','') ?></time>
</div>
</div>
...
$comment_args = array(
	...
	'author'			=> '<div class="form-combo">'.($req ? ' ' : '').'<div class="form-group"><input  placeholder="'.__('Your Name', THEMES_NAMES).'" class="form-control" id="author" name="author" type="text" value="'. esc_attr($commenter['comment_author']).'"  rows="5"  /></div>',
	'email'				=> ($req ? ' ' : '').'<div class="form-group"><input placeholder="'.__('Your Email', THEMES_NAMES).'" class="form-control" id="email" name="email" type="text" value="'.esc_attr($commenter['comment_author_email']).'"  rows="5"  /></div></div>',
	'url'				=> '')),
    'comment_field'		=> '<!--<label for="comment">'.__('Let us know what you have to say:', THEMES_NAMES) . '</label>-->'.'<div class="form-group" id="comment-editor"><div class="bb-editor"><textarea placeholder="'.__('Your Comments', THEMES_NAMES).'" name="comment" id="comment" class="apkmody materialize-textarea has-very-light-gray-background-color comment-form-input no-border" rows="5"></textarea></div></div>',
	'submit_button'		=> '<div class="form-submit"><button name="submit" type="submit" id="submit" class="btn btn-block s-green"  >'.__('Send', THEMES_NAMES).'</button></div>',
	//'submit_field'         => ' ',
    'comment_notes_after' => ' ',
); 
