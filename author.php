...
function show_recent_comments_handler( $atts, $content = null ){
    ...
    if ( is_user_logged_in() ) {
        ...
        $output.= "<p>";
        $output.= __('logged in to see your comments.', THEMES_NAMES);
        $output.= '<a href="'.get_settings('siteurl').'/wp-login.php?redirect_to='.get_settings('siteurl').'">'.__('Login Now', THEMES_NAMES).' &rarr;</a></p>';
    }
    return $output;
}
...
<div class="page-sys page-user">
	...
	<form method="post" name="userinfo" id="userinfo" enctype="multipart/form-data" action="">
		...
		_e('Add Your Biographical Info', THEMES_NAMES); 
		} 
		?>
		</div>
		</div>
