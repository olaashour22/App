...
$status			= strtolower(esc_html( get_post_meta( $post->ID, 'wp_newupdates', true ) )); 
?>
<style><?php if ( 'new' === $status ) { ?>.label-new{color: var(--color_news)!important}<?php } elseif ( 'updates' === $status ) { ?>.label-updates{color: var(--color_updates)!important}<?php } ?><?php if ( 'new' === $status ) { ?>.label-new::before{box-shadow:0 .25rem .5rem 0 var(--color_news)!important}<?php } elseif ( 'updates' === $status ) { ?>.label-updates::before{box-shadow:0 .25rem .5rem 0 var(--color_updates)!important}<?php } ?></style>
...
<div class="entry entry-app">
	<div class="item">
	<?php if ($labels) { ?>
	<span class="label label-<?php if ( 'new' === $status ) { ?>new<?php } elseif ( 'updates' === $status ) { ?>updates<?php } ?>"><?php echo $status; ?></span><?php } else { ?><?php } ?>
		<figure class="img">
		...
		<span class="truncate" style="color: var(--color_link);"><?php global $opt_themes; if($opt_themes['ex_themes_rtl_activate_']) { ?><?php if ($requires) { ?><?php echo RTL_Nums($requires); ?><?php } else { ?><?php echo RTL_Nums(4.5); ?><?php } ?><?php } else { ?><?php if ($requires) { ?><?php echo $requires; ?><?php } else { ?>4.5<?php } ?><?php } ?></span></li>
		<li><svg width="24" height="24"><use xlink:href="#i__vers"></use></svg>
		<span class="truncate" style="color: var(--color_link);"><?php global $opt_themes; if($opt_themes['ex_themes_rtl_activate_']) { ?><?php if (get_post_meta( $post->ID, 'wp_version', true )) { ?><?php echo RTL_Nums( get_post_meta( $post->ID, 'wp_version', true ) ); ?><?php } else { ?><?php echo RTL_Nums( get_post_meta( $post->ID, 'wp_version_GP', true ) ); ?><?php } ?><?php } else { ?><?php if (get_post_meta( $post->ID, 'wp_version', true )) { ?><?php echo esc_html( get_post_meta( $post->ID, 'wp_version', true ) ); ?><?php } else { ?><?php echo esc_html( get_post_meta( $post->ID, 'wp_version_GP', true ) ); ?><?php } ?><?php } ?></span></li>
		</ul>
	</div>
</div>
