...
$types			= 'post';
$sources		= 'apkmody.com';
$demos			= 'https://apkmody.com/games/minecraft';
?>
        ...
        if(isset($_POST['wp_url'])) {
            ...
            if(stristr($url, $sources)) {
				...
				if (empty($check_title) ){
                    ...
                    add_post_meta( $post_id, 'name_downloadlinks_gma', strip_tags($gets_data['name_downloadlink_gma']) );
                    add_post_meta( $post_id, 'name_downloadlinks_gma_1', strip_tags($gets_data['name_downloadlink_gma_1']) );
                    add_post_meta( $post_id, 'name_downloadlinks_gma_2', strip_tags($gets_data['name_downloadlink_gma_2']) );
                    add_post_meta( $post_id, 'name_downloadlinks_gma_3', strip_tags($gets_data['name_downloadlink_gma_3']) );
                    add_post_meta( $post_id, 'name_downloadlinks_gma_4', strip_tags($gets_data['name_downloadlink_gma_4']) );
                    add_post_meta( $post_id, 'name_downloadlinks_gma_5', strip_tags($gets_data['name_downloadlink_gma_5']) );
        ...
        <div class="tab-content">
            ...
            <div id="menu1" class="tab-pane fade">
				...
				<?php  
                    ...
                    @ini_set('user_agent', 'Mozilla/5.0 (compatible; Googlebot/2.1; +https://www.google.com/bot.html)');  
					ini_set('user_agent', 'My-Application/2.5');
						$target1	= 'https://apkmody.com/games';  
						$target2	= 'https://apkmody.com/games/page/2';  
						...
						$item['title']		= trim($article->find('h2.has-normal-font-size.no-margin.no-padding.truncate', 0)->{'plaintext'});
            ...
            <div id="menu2" class="tab-pane fade">
				...
				<?php  
                    ...
                    @ini_set('user_agent', 'Mozilla/5.0 (compatible; Googlebot/2.1; +https://www.google.com/bot.html)');  
					ini_set('user_agent', 'My-Application/2.5');
						$target1	= 'https://apkmody.com/apps';  
                        $html		= file_get_html($target1, false, stream_context_create($ssl));   
						$target2	= 'https://apkmody.com/apps/page/2';  
                        $html2		= file_get_html($target2, false, stream_context_create($ssl));   
                        foreach($html->find("div.flex-container article") as $article) {
						$item['title']		= trim($article->find('h2.has-normal-font-size.no-margin.no-padding.truncate', 0)->{'plaintext'});
						$item['url']		= trim($article->find('a.app.clickable', 0)->href) ;
