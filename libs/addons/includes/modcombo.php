...
function wp_modcombo_errors() {
$sources        = 'modcombo.com';
?>
...
function wp_modcombo() {
$types			= 'post';
$sources		= 'modcombo.com';
$demos			= 'https://modcombo.com/free-fire.html'; 
?>
    ...
    if(isset($_POST['wp_sb'])) {
        ...
        if(isset($_POST['wp_url'])) {
            ...
            if(stristr($url, $sources)) {
                require_once 'modcombo.class.php';
				########## 
				global $opt_themes;
				...
				if (empty($check_title) ){
					...
					*/
					...
					if ($gets_data['downloadlinks_gma'][0]) { 
                    add_post_meta( $post_id, 'type_downloadlinks_gma', $gets_data['type_downloadlink_gma'] );
					}
					if ($gets_data['downloadlinks_gma'][1]) { 
                    add_post_meta( $post_id, 'type_downloadlinks_gma_1', $gets_data['type_downloadlink_gma_1'] );
					}
					if ($gets_data['downloadlinks_gma'][2]) { 
                    add_post_meta( $post_id, 'type_downloadlinks_gma_2', $gets_data['type_downloadlink_gma_2'] );
					}
					if ($gets_data['downloadlinks_gma'][3]) { 
                    add_post_meta( $post_id, 'type_downloadlinks_gma_3', $gets_data['type_downloadlink_gma_3'] );
					}
					if ($gets_data['downloadlinks_gma'][4]) { 
                    add_post_meta( $post_id, 'type_downloadlinks_gma_4', $gets_data['type_downloadlink_gma_4'] );
					}
					if ($gets_data['downloadlinks_gma'][5]) { 
                    add_post_meta( $post_id, 'type_downloadlinks_gma_5', $gets_data['type_downloadlink_gma_5'] );
					} 
        ...
        <div class="tab-content">
            ...
            <div id="menu4" class="tab-pane fade in active">
              ...
              <div class="play_menu" style="text-transform:uppercase">
                <ol style="paddiing-right: 20px !important;margin: 20px;">
                    <li>Open website <a style="color:<?php echo colors_rgb; ?>" href="https://<?php echo $sources; ?>" target="_blank"><?php echo $sources; ?></a></li>
			...
			<section>
				<div class="row">
				 <?php				
                    ...
                    @ini_set('user_agent', 'Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)');   
						$target1	= 'https://modcombo.com/games/?sort=new'; 
						$target2	= 'https://modcombo.com/games/page/2?sort=new'; 
                        $html		= file_get_html($target1, false, stream_context_create($ssl));
                        $html2		= file_get_html($target2, false, stream_context_create($ssl));
                        foreach($html->find("ul.apks.w3.apkincategories.box-space li") as $article) {
						$item['title']		= trim($article->find('div.title.truncate', 0)->{'plaintext'});
						$item['url']		= trim($article->find('a.apk-icon', 0)->href) ;
						$item['version']	= trim($article->find('div.version.truncate', 0)->{'plaintext'});
						$item['mod']		= trim($article->find('div.mod', 0)->{'data-mod-feature'});
						$item['img']		= trim($article->find('img', 0)->{'data-src'}); 
						  
						$ret[]				= $item;
						}
                        foreach($html2->find("ul.apks.w3.apkincategories.box-space li") as $article) {
						$item['title']		= trim($article->find('div.title.truncate', 0)->{'plaintext'});
						$item['url']		= trim($article->find('a.apk-icon', 0)->href) ;
						$item['version']	= trim($article->find('div.version.truncate', 0)->{'plaintext'});
						$item['mod']		= trim($article->find('div.mod', 0)->{'data-mod-feature'});
				...
				<div class="col-12 col-md-6 mb-4">
							...
							<img src="<?php echo $src; ?>" alt="<?php echo $GP_['title']; ?> <?php echo $GP_['version']; ?> <?php echo base64_decode($GP_['mod']); ?>" class="app-logo " width="64" height="64"> 
							</div>
							
							<div class="app-info">
							<h3 class="h5 font-weight-semibold w-100 app-title"><?php echo $GP_['title']; ?></h3>
							<div class="text-truncate text-muted app-desc">			
							<span class="clamp-1 w-100"> <?php echo base64_decode($GP_['mod']); ?></span>
				...
				<div class="row">		 
				 <?php				
                    ...
                    @ini_set('user_agent', 'Mozilla/5.0 (compatible; Googlebot/2.1; +http://www.google.com/bot.html)');   
						...
						$target1	= 'https://modcombo.com/apps';  
                        $html		= file_get_html($target1, false, stream_context_create($ssl)); 
                        foreach($html->find("ul.apks.w3.apkincategories.box-space li") as $article) {
						$item['title']		= trim($article->find('div.title.truncate', 0)->{'plaintext'});
						$item['url']		= trim($article->find('a.apk-icon', 0)->href) ;
