...
class starting_now{	
	...
	public function get_webs_info_by_id($getLinkID) {
		...
		$sources = "https://getmodsapk.com/".trim($getLinkID)."/";
		} 
	...
	public function scrape_web_info($sources) {				
		...
		$arr['title_id'] 			= $this->match('/<a href=".*?\?id=(.*?)\&.*?".*?>.*?<img src=".*?google-play-min.png".*?>.*?<\/a>/ms', $links, 1);		
		
		$arr['title_id_alt']		= $this->match('/<a href=".*?\?id=(.*?)".*?>.*?<img src=".*?google-play-min.png".*?>.*?<\/a>/ms', $links, 1);
		...
		$arr['title']				= str_replace(array('getmodsapk.com', '&#8211;', '-', 'free', 'for Android', '+', 'Download'), '', $this->match('/<title>(.*?)<\/title>/ms', $links, 1));
		$arr['title']				= preg_replace('/\s+/', ' ', $arr['title']);
		 
		$arr['title_web']			= $this->match('/<\/svg>.*?App Name.*?<td.*?>(.*?)<\/td>/ms', $links, 1);		
		$title_webs					= $arr['title_web'];
		
		$arr['title2']				= str_replace(array('getmodsapk.com', '&#8211;', '-', 'free', 'for Android', '+', 'Download'), '', trim(strip_tags($this->match('/<span class="sm:text-left text-center block w-full">(.*?)<\/span>/ms', $links, 1))));
		...
		$arr['article_content']		= $this->match('/<div class="sm:mt-5 mt-10 leading-relaxed.*?post-description.*?">(.*?)<\/div>/ms', $links, 1);
		$arr['article_content']		= preg_replace('/<a.*?>(.*?)<\/a>/is', '',  $arr['article_content']);
		$arr['article_content']		= preg_replace('/<h(.*?) id=".*?">(.*?)</is', '<h$1>$2 '.$arr['title'].'<',  $arr['article_content']);
		...
		/*		https://files.getmodsapk.com/ROOT//All%20Documents%20File/Folder%2016/Mech%20Arena//Mech-Arena-v2.28.00-MOD-MENU-GMA-(Getmodsapk.com).apk
		*/
		if ($arr['downloadlinks_gma'][0]) {  
		$arr['type_downloadlink_gma']		= $this->match('/<div class="mt-4.*?>.*?<a href=".*?\).(.*?)".*?<\/a>.*?<\/div>/ms', $download_links_pages_alt, 1);
		}
		if ($arr['downloadlinks_gma'][1]) {  
		$arr['type_downloadlink_gma_1']		= $this->match('/<div class="mt-4.*?>.*?<a href=".*?\).(.*?)".*?<\/a>.*?<\/div>/ms', $download_links_pages_alt_1, 1);
		}
		if ($arr['downloadlinks_gma'][2]) {  
		$arr['type_downloadlink_gma_2']		= $this->match('/<div class="mt-4.*?>.*?<a href=".*?\).(.*?)".*?<\/a>.*?<\/div>/ms', $download_links_pages_alt_2, 1);
		}
		if ($arr['downloadlinks_gma'][3]) {  
		$arr['type_downloadlink_gma_3']		= $this->match('/<div class="mt-4.*?>.*?<a href=".*?\).(.*?)".*?<\/a>.*?<\/div>/ms', $download_links_pages_alt_3, 1);
		}
		if ($arr['downloadlinks_gma'][4]) { 
		$arr['type_downloadlink_gma_4']		= $this->match('/<div class="mt-4.*?>.*?<a href=".*?\).(.*?)".*?<\/a>.*?<\/div>/ms', $download_links_pages_alt_4, 1);
		}
		if ($arr['downloadlinks_gma'][5]) { 
		$arr['type_downloadlink_gma_5']		= $this->match('/<div class="mt-4.*?>.*?<a href=".*?\).(.*?)".*?<\/a>.*?<\/div>/ms', $download_links_pages_alt_5, 1);
		} 
