...
function breadcrumbsX() {
	global $opt_themes; 
	$home_text		= $opt_themes['text_home_bcm'];
    $linkXhost		= get_bloginfo('url'); 
	$parse			= parse_url($linkXhost); 
	$watermark1		= $parse['host'];
    /* === OPTIONS === */
    $text['home']     = __($home_text, TEXT_DOMAIN ); // text for the 'Home' link
    $text['category'] = '%s'; // text for a category page
    $text['search']   = '%s'; // text for a search results page
    $text['tag']      = '%s'; // text for a tag page
    $text['author']   = '%s'; // text for an author page
    $text['404']      = __( 'Error 404', TEXT_DOMAIN ); // text for the 404 page
    $text['page']     = __( 'Page %s', TEXT_DOMAIN ); // text 'Page N'
    $text['cpage']    = __( 'Comment Page %s', TEXT_DOMAIN ); // text 'Comment Page N'
    $wrap_before    = '<span itemscope itemtype="http://schema.org/BreadcrumbList">'; // the opening wrapper tag
