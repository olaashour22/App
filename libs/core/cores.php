...
global $opt_themes;
?>
<img src="<?php if($opt_themes['ex_themes_logo_headers_active_']) { echo $opt_themes['header_logo']['url']; } else { echo EX_THEMES_URI; ?>/assets/img/logos.png<?php } ?>" alt="<?php echo get_option("blogname") ?>" width="104" height="36" />
<?php 
}
...
<a href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo get_option("blogname") ?></a> - <a href="<?php echo EXTHEMES_ITEMS_URL; ?>" title="Using Themes <?php echo THEMES_NAMES; ?>.v<?php echo VERSION; ?> Premium by <?php echo DEVS; ?>"><?php echo THEMES_NAMES; ?>.v<?php echo VERSION; ?></a> © <script type="text/javascript">var creditsyear = new Date();document.write(creditsyear.getFullYear());</script> <?php _e('All rights reserved - Developed by', THEMES_NAMES); ?> <a href="<?php echo EXTHEMES_API_URL; ?>" title="<?php _e('Premium Themes Wordpress ', THEMES_NAMES); ?> - <?php echo DEVS; ?>"><?php echo DEVS; ?></a>
<?php }
}
...
function ex_themes_related_posts_() { 
	...
	<section class="wrp section section-related">
        <div class="section-head">
            <h3 class="section-title"><i class="s-blue c-icon"><svg width="24" height="24"><use xlink:href="#i__explore"></use></svg></i><?php _e($opt_themes['exthemes_more_by_developers'], THEMES_NAMES); ?></h3>
				<?php
                global $post;
    ...
    <section class="wrp section section-related">
        <div class="section-head">
        <h3 class="section-title"><i class="s-blue c-icon"><svg width="24" height="24"><use xlink:href="#i__explore"></use></svg></i><?php _e($titles, THEMES_NAMES); ?></h3>
        <?php
        $category = get_the_category();
        echo '<a class="btn s-green btn-all" href="'.get_category_link($category[0]->cat_ID).'"><span>'.__( 'All', THEMES_NAMES ).' '. $category[0]->cat_name.'</span><svg width="24" height="24"><use xlink:href="#i__keyright"></use></svg></a>';
        ?>
        </div>
        <div class="entry-list list-c6">
        <?php
...
<div class="b-head">
<h3 class="section-title fbold"><i class="s-blue c-icon"><svg width="24" height="24"><use xlink:href="#i__vers"></use></svg></i> <?php _e($opt_themes['title_latest_version'], THEMES_NAMES); ?></h3>
</div>
		...
		<div class="list">
            <div class="package_info open_info">
                ...
                <span class="name"><?php if ($title) { echo ucwords($title); } ?></span>
                    ...
                    <span class="<?php if($mods){ ?>mod<?php } else { ?>apk<?php } ?>"><?php if($mods){ ?><?php _e($opt_themes['title_version_mod'], THEMES_NAMES); ?><?php } else { ?><?php _e($opt_themes['title_version_apk'], THEMES_NAMES); ?><?php } ?></span>
                </div>
                <div class="text">
            ...
            <div class="v_h_button button_down ">
                <a class="down" href="<?php the_permalink() ?>"><span><?php _e($opt_themes['exthemes_apk_info_Download'], THEMES_NAMES); ?></span></a>
            </div>
		</div>
		<?php endwhile; wp_reset_query(); ?>
        
        <a class="more-version" href="<?php the_permalink(); ?>version" style="cursor: pointer;display: block; width: 100%;text-align: center; border-radius: 4px;text-transform: uppercase;  font-size: 14px;  line-height: 40px;">All Versions</a>
...
<?php } }}
add_shortcode('version_pages', 'version_pages');

function get_version_titles(){
...
<title>All History Versions of <?php echo ucwords($title); ?> on Android  - <?php echo strtoupper($situs); ?></title>

<?php }
}
add_action( 'wp_head', 'get_version_titles',0); 
...
?>
    <div class="block b-screens">
        <div class="b-icon-title">
            ...
            <h3 class="b-title"><?php _e($opt_themes['exthemes_Screenshots'], THEMES_NAMES); ?></h3>
        </div>
        <div class="b-cont">
            ...
            <a href="<?php echo $gallery['image_url'][$i] ; ?>" class="highslide" ><img  alt="" src="<?php echo $gallery['image_url'][$i] ; ?>" style="max-width:100%;"></a>
            <?php }
            }
            endif;
            } else { 
            ...
            $judul  = get_post_meta(get_the_ID(), 'wp_title_GP', true);
            $i          = 0;
            $count      = 4;
            foreach($datos_imagenes as $elemento) {
            $i++;
            if ( $i <= $count ) { ?>
            <a href="<?php echo (!empty($datos_imagenes[$i])) ? $datos_imagenes[$i] : ''; ?>" class="highslide" ><img alt="" src="<?php echo (!empty($datos_imagenes[$i])) ? $datos_imagenes[$i] : ''; ?>" style="max-width:100%;"></a>
            <?php } } 
            }} 
            } ?>
            </div>
...
global $opt_themes;
?>    
<h1 class="atitle has-text-centered"><?php if($opt_themes['ex_themes_homes_titles_']) { ?><?php echo $opt_themes['ex_themes_homes_titles_']; ?><?php } else { ?><?php echo get_option("blogname") ?><?php } ?></h1> 
<?php }
// ~~~~~~~~~~~~~~~~~~~~~ EX_THEMES ~~~~~~~~~~~~~~~~~~~~~~~~ \\
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-head">
                    ...
                    <h3 class="modal-title" ><?php _e($opt_themes['exthemes_Login_to'], THEMES_NAMES); ?> <?php echo $watermark1; ?></h3>
                    <?php
                ...
                <div class="modal-body">
                    <form name="loginform" id="loginform" class="login_form" method="post" action="<?php echo site_url( '/wp-login.php' ); ?>">
                        <label class="form-group">
                            <span class="c-muted"><?php _e($opt_themes['exthemes_Logins'], THEMES_NAMES); ?></span>
                            <input class="form-control" type="text" name="log" id="user_login" required>
                        </label>
                        <label class="form-group">
                            <a class="f-right" href="<?php echo wp_lostpassword_url( ); ?>"><?php _e($opt_themes['exthemes_Forgot_your_password'], THEMES_NAMES); ?></a>
                            <span class="c-muted"><?php _e($opt_themes['exthemes_Password'], THEMES_NAMES); ?></span>
                        ...
                        <div class="form-submit btn-group">
                            <button class="btn btn-block s-green" onclick="submit();" name="wp-submit" id="wp-submit" type="submit"><?php _e($opt_themes['exthemes_Logins'], THEMES_NAMES); ?></button>
                            <a href="<?php echo esc_url( home_url( '/' ) ); ?>register/" class="btn btn-block s-yellow"><?php _e($opt_themes['exthemes_Registration'], THEMES_NAMES); ?></a>
                        </div>
                    </form>
                </div>
            </div>
