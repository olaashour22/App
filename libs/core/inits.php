...
if ( ! function_exists( 'play5_setup' ) ) :
    function play5_setup() {
		...
		load_theme_textdomain( '5play', get_template_directory() . '/languages' );
...
if ( function_exists('register_sidebar') )
    register_sidebar(array(
        'id'					=> 'intros-homes',
        'name'					=> TEXT_DOMAIN.' intros Home',
        'description'			=> __( 'Widgets in this area will be shown on intros Home', TEXT_DOMAIN ),
...
if ( function_exists('register_sidebar') )
    register_sidebar(array(
        'id'					=> 'recommend-homes',
        'name'					=> TEXT_DOMAIN.' Recommend Home',
        'description'			=> __( 'Widgets in this area will be shown on Recommend Home', TEXT_DOMAIN ),
        'before_widget'			=> '<section class="wrp section section-recom">',
        'after_widget'			=> '</section>',
        'before_title'			=> '<h3 class="section-title"><i class="s-green c-icon"><svg width="24" height="24"><use xlink:href="#i__hot"></use></svg></i>',
        'after_title'			=> '</h3>',
...
if ( function_exists('register_sidebar') )
    register_sidebar(array(
        'id'					=> 'home-popular',
        'name'					=> TEXT_DOMAIN.' Home Categories',
		'description'			=> __( 'Widgets in this area will be shown only Home Categories', TEXT_DOMAIN ),
...
/* if ( function_exists('register_sidebar') )
    register_sidebar(array(
        'id' => 'home-news',
        'name'=> ''.TEXT_DOMAIN.' Home News',
		'description'   => __( 'Widgets in this area will be shown only Home News', TEXT_DOMAIN ),
...
/* if ( function_exists('register_sidebar') )
    register_sidebar(array(
        'id' => 'home-comments',
        'name'=> ''.TEXT_DOMAIN.' Home Comments',
		'description'   => __( 'Widgets in this area will be shown only Home Comments', TEXT_DOMAIN ),
...
if ( function_exists('register_sidebar') )
    register_sidebar(array(
        'id'					=> 'home-footers',
        'name'					=> TEXT_DOMAIN.' Home Footers',
		'description'			=> __( 'Widgets in this area will be shown only Home Footers', TEXT_DOMAIN ),
        'before_widget'			=> '<section class="wrp section section-news">',
        'after_widget'			=> '</section>',
        'before_title'			=> '<div class="section-head"><h3 class="section-title"><i class="s-yellow c-icon"><svg width="24" height="24"><use xlink:href="#i__gamepad"></use></svg></i>',
        'after_title'			=> '</h3></div>',
...
function ex_themes_duplicate_callback() {
    function ex_themes_results_checks() {
        ...
        if($results) {
            return '<div class="error"><p><span class="dashicons dashicons-warning"></span> '. __( 'This content already exists, we recommend not to publish.' , TEXT_DOMAIN ) .' </p></div>';
        } else {
            return '<div class="notice rebskt updated"><p><span class="dashicons dashicons-thumbs-up"></span> '.__('Excellent! this content is unique.' , TEXT_DOMAIN).'</p></div>';
        }
    }
...
add_rewrite_endpoint( 'version', EP_PERMALINK | EP_PAGES );
function ex_themes_version() {
    add_rewrite_endpoint( 'version', EP_PERMALINK | EP_PAGES );
}
add_action( 'init', 'ex_themes_version' );
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  @EXTHEM.ES  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ \\
function ex_themes_version_template() {
    global $wp_query;
    // if this is not a request for play or a singular object then bail
    if ( ! isset( $wp_query->query_vars['version'] ) || ! is_singular() )
        return;
    // include custom template
    include get_template_directory().'/template/version.php';
    exit;
}
add_action( 'template_redirect', 'ex_themes_version_template' );
// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  @EXTHEM.ES  ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ \\
...
function exthemes_news_likes() {
    ...
    if(is_singular('news')) {
        ...
        <div class="likes">
			...
			<span class="sr-only"><?php if($opt_themes['exthemes_Like']) { echo $opt_themes['exthemes_Like']; } ?></span>
            </a>
            <a href="<?php echo add_query_arg('post_action', 'dislike'); ?>">
			<span class="like-minus"><svg width="24" height="24"><use xlink:href="#i__thumbdown"></use></svg>-<span  class="ignore-select"><?php echo exthemes_post_likes_count('dislikes') ?></span><span class="sr-only"><?php if($opt_themes['exthemes_Dislike']) { echo $opt_themes['exthemes_Dislike']; } ?></span></span>
            </a>
        </div>
        <?php
        $output = ob_get_clean();
...
function exthemes_news_likes_count($type = 'likes') {
	...
	if ($current_count > 1000) {
		return round ( $current_count / 1000 , 1 ).'K';
	} else {
    ...
    <div id='comment-id-<?php comment_ID(); ?>'>
        <div class="comment ">
            <div class="comment-head">
                ...
                <span class="name truncate"><a><?php $cID = $comment->comment_ID; printf( __( '%2$s', TEXT_DOMAIN ), get_comment_author_url($cID), get_comment_author($cID), get_comment_link($cID), get_comment_date('',$cID) ); ?></a></span>
                <div class="comment-meta">
                    <time class="date c-muted" datetime="<?php printf(  __( '%1$sT%2$s', TEXT_DOMAIN ), get_comment_date(), get_comment_time() ); ?>"><?php printf(  __( '%1$s', TEXT_DOMAIN ), get_comment_date(), get_comment_time() ); ?> <?php edit_comment_link(__('(Edit)'),'  ', TEXT_DOMAIN) ?></time>
                </div>
            </div>
...
function bg_recent_comments($no_comments = 3, $comment_len = 100, $avatar_size = 60) {
    ...
    if ( $comments ) : foreach ( $comments as $comment ) :
        ...
        $comm .= sprintf(  __( '<time class="date muted" datetime="%1$sT%2$s" style="var(--opacity)!important;color: white !important;">%1$s</time>', '%s = human-readable time difference', TEXT_DOMAIN ), get_comment_date(), get_comment_time() );
        $comm .= '<i class="entry-coms-reply"><svg width="24" height="24"><use xlink:href="#i__reply"></use></svg></i>';
        $comm .= '</div></div>';
    endforeach; else :
        $comm .= __( 'No comments.', TEXT_DOMAIN );
    endif;
    echo $comm;
}
...
if ( false === $last_login ) {
    $last_login = __( 'Never', TEXT_DOMAIN );
} else {
    $last_login = date( 'j F Y, H:i:s', $last_login );
}
echo '<p>'.__( 'Last Login :', TEXT_DOMAIN ).' '.$last_login.'</p>';
...
function gearside_user_columns_content($value='', $column_name, $id){
    if ( $column_name == 'status' ){
		if ( gearside_is_user_online($id) ){
			return '<strong style="color: green;">'.__( 'Online Now', TEXT_DOMAIN ).'</strong>';
		} else {
			return ( gearside_user_last_online($id) )? '<small>'.__( 'Last Seen:', TEXT_DOMAIN ).' <br /><em>' . date('M j, Y @ g:ia', gearside_user_last_online($id)) . '</em></small>' : ''; //Return the user's "Last Seen" date, or return empty if that user has never logged in.
		}
	}
}
...
add_shortcode( 'list_subcats', function() {
	...
	echo '<li><a href="'.esc_url( get_category_link( $cats->term_id ) ).'">'.$cats->name.'</a></li>';
    }
