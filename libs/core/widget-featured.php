...
class rekomended_widget extends WP_Widget {
	...
	function widget( $args, $instance ){
		...
		<h2 class="title">dsdsadsad<a class="item-link" href="<?php the_permalink() ?>"><span class="truncate"><?php if ($title) { if($appname_on) { echo ucwords($title); } else { echo $title_alt; } } else { echo $title_alt; } ?></span></a></h2>
	...
	function form( $instance ){
		...
		<p>
			<label for="<?php echo $this->get_field_id('postids'); ?>"><?php _e('Post ID', THEMES_NAMES); ?> </label>
			...
			<br />
			<small>
			<?php _e( 'Post IDs, separated by commas.', THEMES_NAMES); ?>
			<br />
			<?php _e( 'Only Limit 4 Post', THEMES_NAMES); ?>
			</small>
		</p>
		<?php
	}
