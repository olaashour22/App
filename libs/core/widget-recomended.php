...
class recommend_widget extends WP_Widget {
	...
	function widget( $args, $instance ){
		...
		<h2 class="section-title"><i class="s-green c-icon"><svg width="24" height="24"><use xlink:href="#i__hot"></use></svg></i><?php echo $instance['title']; ?></h2>	
	...
	function form( $instance ){
		...
		?>
		<p style="text-align: center;font-weight: bold;"><?php _e('Recommend Widgets ', THEMES_NAMES); ?></p>
		<hr />
		<p>
		...
		<hr />
		<p>
		<label for="<?php echo $this->get_field_id('postids'); ?>"><?php _e( 'Post ID Number', THEMES_NAMES); ?></label>
		...
		<br />
		<pre><?php _e( 'Post ID Number, Separated by Commas.', THEMES_NAMES); ?> <br><?php _e( 'Only limited to 4 Posts', THEMES_NAMES); ?></pre>
		</p>
		<hr />
		<?php
	}
